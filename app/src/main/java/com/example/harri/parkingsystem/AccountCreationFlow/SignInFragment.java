...
View rootView=        inflater.inflate(R.layout.fragment_sign_in, container, false);
                    ...
                    .addOnCompleteListener(getActivity(), new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if(!task.isSuccessful()){
                                ...
                                Toast.makeText(getActivity(),"temp",
                                        ...
                                        public void onDataChange(DataSnapshot dataSnapshot) {
                                            if (dataSnapshot.exists()){
                                               ...
                                               membershipType= myUser.getMemberType();
                                                saveUserPref();
                                                if (myUser.getMemberType().equals("User")){
                                                    
                                                    openUserActivity(v);
                                                    
                                                }else{
                                                    
                                                    if (myUser.getMemberType().equals("Admin")){
                                                        openAdminActivity(v);
                                                        
                                                    } else{


                                                        Toast.makeText(getActivity(), "temp2",
                                                                Toast.LENGTH_LONG).show();
    ...
    private void openAdminActivity(View view) {
        mProgressDialog.dismiss();
        Toast.makeText(getActivity(), getString(R.string.admin_logged),
                Toast.LENGTH_LONG).show();
        Toast.makeText(getActivity(), getString(R.string.admin_logged),
                Toast.LENGTH_LONG).show();
